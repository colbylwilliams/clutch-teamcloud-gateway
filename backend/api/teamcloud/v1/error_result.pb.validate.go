...
// ValidateAll checks the field values on ErrorResult with the rules defined in
// the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in ErrorResultMultiError, or
// nil if none found.
func (m *ErrorResult) ValidateAll() error {
	return m.validate(true)
}

func (m *ErrorResult) validate(all bool) error {
	if m == nil {
		return nil
	}
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, ErrorResultValidationError{
	...
	if len(errors) > 0 {
		return ErrorResultMultiError(errors)
	}

	return nil
}

// ErrorResultMultiError is an error wrapping multiple validation errors
// returned by ErrorResult.ValidateAll() if the designated constraints aren't met.
type ErrorResultMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m ErrorResultMultiError) Error() string {
	var msgs []string
	for _, err := range m {
...
func (m ErrorResultMultiError) AllErrors() []error { return m }
