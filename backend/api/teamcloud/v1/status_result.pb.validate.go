...
// ValidateAll checks the field values on StatusResult with the rules defined
// in the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in StatusResultMultiError, or
// nil if none found.
func (m *StatusResult) ValidateAll() error {
	return m.validate(true)
}

func (m *StatusResult) validate(all bool) error {
	if m == nil {
		return nil
	}
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, StatusResultValidationError{
			...
			if err := v.Validate(); err != nil {
				return StatusResultValidationError{
					field:  fmt.Sprintf("Errors[%v]", idx),
					reason: "embedded message failed validation",
					cause:  err,
				}
	...
	if m.UnderscoretrackingId != nil {
		// no validation rules for UnderscoretrackingId
	}

	if len(errors) > 0 {
		return StatusResultMultiError(errors)
	}

	return nil
}

// StatusResultMultiError is an error wrapping multiple validation errors
// returned by StatusResult.ValidateAll() if the designated constraints aren't met.
type StatusResultMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m StatusResultMultiError) Error() string {
	var msgs []string
	for _, err := range m {
...
func (m StatusResultMultiError) AllErrors() []error { return m }
